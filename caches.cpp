...
void two_level_cache_wb_full_run(unsigned int *arr1, int *marked_arr1, unsigned int *arr2, int *marked_arr2, int *dirty, unsigned int address, int *counter1, int *counter2, int *access_counter, int target_bits1, int offset_bits1, int set_size1, int target_bits2, int offset_bits2, int set_size2, string type);
...
int main(int argc, char *argv[]) {
    ...
    while(infile >> std::hex >> type >> addr ) {
        ...
        two_level_cache_wb_full_run(cache6c, marked6c, cache6d, marked6d, dirtyb, addr, &(counters[14]), &(counters[15]), &(access_counter[3]), 5, 5, 4, 13, 6, 1024, type);
        //write back directly-mapped
        two_level_cache_wb_run(cache6a, marked6a, cache6b, marked6b, dirtya, addr, &(counters[16]), &(counters[17]), &(access_counter[4]), 5, 5, 4, 10, 6, 1, type);
        //l2 accessess
    ...
    outfile << counters[3] << "," << access_counter[3]  << ";"; //this is how the expected file is formatted
    outfile << endl;

    for(int i=0; i<3;i++){
        outfile << counters[4+i] << "," << access_counter[0]  << "; ";
    }
    outfile << counters[7] << "," << access_counter[3]  << ";"; //this is how the expected file is formatted
    outfile << endl;

    outfile << counters[8] << "," << access_counter[0]  << "; ";
    outfile << counters[9] << "," << access_counter[0]  << ";";
    outfile << endl;
...
void two_level_cache_wb_full_run(unsigned int *arr1, int *marked_arr1, unsigned int *arr2, int *marked_arr2, int *dirty, unsigned int address, int *counter1, int *counter2, int *access_counter, int target_bits1, int offset_bits1, int set_size1, int target_bits2, int offset_bits2, int set_size2, string type){
    ...
    int hit = set_size1;
    while(marked_arr1[(index1*set_size1) + i] != 0 && i<set_size1){
        if(arr1[(index1*set_size1)+(i)] == block1){
            hit = i;
        }
        i++;
    }
    if(hit < set_size1){
        ...
        if(dirty[index1*set_size1 + lru] == 1){
            ...
            int i = 0;
            int hit = set_size2;
            while(marked_arr2[i] != 0 && i<set_size2){
                if(arr2[ (i)] == block3){
                    hit = i;
                }
                i++;
